...
- **依赖**：TASK-002
- **验收标准**：
  - [x] 实现规则数据库模型（SQLite/PostgreSQL）
  - [x] 提供 CRUD API：create, read, update, delete, list
  - [x] 支持规则版本号
  - [x] 支持按 category/risk_level 过滤
- **自测要求**：
  - [x] 创建 10+ 条测试规则
  - [x] 验证 CRUD 操作正确性
  - [x] 测试过滤查询功能
...
- **验收标准**：
  ...
  - [x] GET /api/rules - 获取规则列表（支持过滤）
  - [x] GET /api/rules/{rule_id} - 获取单条规则
  - [x] PUT /api/rules/{rule_id} - 更新规则
  - [x] DELETE /api/rules/{rule_id} - 删除规则
- **自测要求**：
  - [x] 使用 curl/Postman 测试所有接口
  - [x] 验证请求/响应符合 Schema
  - [x] 测试异常处理（404, 400）
...
- **评估结果**：
  - rule_parsing.txt: 20/20 通过 (100%)
  - review.txt: 19/19 通过 (100%)
  - explain.txt: 19/19 通过 (100%)
  - 总计: 58/58 通过 (100%)
...
- **自测要求**：
  - [x] 文档完整性检查
- **输出文件**：
  - `docs/deployment.md` - 完整部署文档（系统要求、快速开始、详细步骤、Docker部署、故障排查等）
  - `docs/faq.md` - 常见问题文档（30个FAQ，覆盖安装、使用、功能、错误排查、性能优化等）
...
- **依赖**：TASK-601
- **验收标准**：
  - [x] 准备 3+ 份演示合同（已有2份测试合同，文档中说明第3份准备方案）
  - [x] 配置 10+ 条审核规则（详细配置10条核心规则）
  - [x] 准备演示脚本（完整7步演示流程）
  - [x] 录制 Demo 视频（可选，提供视频脚本）
- **自测要求**：
  - [x] 演示脚本完整性检查
- **输出文件**：`docs/demo_script.md`
- **演示内容**：
  - 10-15分钟完整演示流程
  - 7个演示步骤（上传合同、创建规则、启动审核、查看结果、Evidence定位、追问解释、总结）
  - 10条核心审核规则配置
  - 5个常见问题应对方案
  - 备用方案和视频脚本
